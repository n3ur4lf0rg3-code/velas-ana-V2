  ...
  return (
    <main>
      <section className="mx-auto grid max-w-6xl items-center gap-10 px-4 py-10 sm:px-6 lg:grid-cols-2 lg:gap-16 lg:py-16">
        ...
        <Reveal delay={1} className="overflow-hidden rounded-2xl bg-surface">
          <img
            src="/products/hero.jpg"
            alt="Mesa de atelier con velas rosa, un frappé de cera y wax melts"
            className="ken-burns aspect-photo w-full object-cover sm:aspect-square lg:aspect-photo"
          />
        </Reveal>
      </section>
      ...
      <section className="bg-surface">
        <div className="mx-auto max-w-6xl px-4 py-16 sm:px-6">
          ...
          <div className="mt-10 grid gap-4 sm:grid-cols-2">
            {SHAPES.map((shape, i) => (
              <Reveal key={shape.id} delay={(i % 3) as 0 | 1 | 2}>
                <Link
                  to="/catalogo"
                  search={{ forma: shape.id }}
                  className="group flex items-center justify-between gap-4 rounded-xl border border-border bg-raised px-6 py-7 transition-all duration-200 hover:border-gold hover:shadow-sm active:scale-[0.99]"
                >
                  <div>
                    <h3 className="font-display text-title">{shape.label}</h3>
                    <p className="mt-1 text-sm text-muted">{SHAPE_BLURBS[shape.id]}</p>
                  </div>
                  <ArrowRight className="size-5 shrink-0 text-gold transition-transform duration-200 group-hover:translate-x-1" />
                </Link>
              </Reveal>
            ))}
          </div>
      ...
      <section className="mx-auto grid max-w-6xl items-center gap-10 px-4 py-16 sm:px-6 lg:grid-cols-2">
        <Reveal className="overflow-hidden rounded-2xl bg-surface">
          <img
            src="/products/hero.jpg"
            alt="Detalle de velas artesanales sobre lino crema"
            className="aspect-wide w-full object-cover object-bottom transition-transform duration-700 hover:scale-[1.03]"
          />
        </Reveal>
        <Reveal delay={1}>
